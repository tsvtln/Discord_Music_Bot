...
class Player(BotRunner, VARS):
    ...
    async def get_video_name(self, youtube_url):  # async wrapper for non-blocking
        ...
        return await loop.run_in_executor(None, functools.partial(Helpers.get_video_name_sync, self.youtube_url))
    ...
    async def play_next_song(self):
        if self.guild_id in self.song_queues and self.song_queues[self.guild_id]:
            ...
            info = await loop.run_in_executor(None,
                                              functools.partial(self.ytdl.extract_info, self.next_url, self.download))
            audio_url = info['url']
